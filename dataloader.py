...
class BalancedFewShotDataloader():
    ...
    def sample_train_and_test_examples_for_base_categories(self, Kbase, nTestBase, nExemplars):
        """
        Sample `nTestBase` number of images from the `Kbase` categories.
        ...
        for Kbase_idx in range(len(Kbase)):
            ...
            imds_tbase = imd_ids[:nEvalExamplesPerClass]
            imds_exemplars = imd_ids[nEvalExamplesPerClass:]

            Tbase += [(img_id, Kbase_idx) for img_id in imds_tbase]
            Exemplars += [(img_id, Kbase_idx) for img_id in imds_exemplars]
        assert(len(Tbase) == nTestBase)
        assert(len(Exemplars) == len(Kbase) * nExemplars)
        random.shuffle(Exemplars)
        
        return Tbase, Exemplars       
