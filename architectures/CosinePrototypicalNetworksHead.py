...
def cosineSimilarity(A, B):
    ...
    A_norm = F.normalize(A, p=2, dim=2, eps=1e-12)
    B_norm = F.normalize(B, p=2, dim=2, eps=1e-12)

    # AB = A * B = [nB x Na x nC] * [nB x nC x Nb] = [nB x Na x Nb]
    cosSim = torch.bmm(A_norm, B_norm.transpose(1, 2))
...
class PrototypicalNetwork(nn.Module):
    ...
    def forward(self, features_test, features_train, labels_train):
        ...
        scores_cls = self.scale_cls * cosineSimilarity(features_test, prototypes)
        return scores_cls
